...
class GenresTableViewController_NEW: UITableViewController {
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		...
		{
			let genre = realm.objects(itemType).filter("name = %@", genreName).first! as! Genre
			let songs = Array(genre.songs)
			dvc.objects = Array(Set(songs.map { $0.artist! }))
		}
	}
